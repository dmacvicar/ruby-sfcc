...
static VALUE key(VALUE self, VALUE name)
{
  ...
  if ( !status.rc )
    return sfcc_cimdata_to_value(&data, rso->client, false);
...
static VALUE each_key(VALUE self)
{
  ...
  if (!status.rc) {
    for (; k < num_props; ++k) {
      ...
      if (!status.rc) {
        rb_yield_values(2, rb_str_intern(CIMSTR_2_RUBYSTR(key_name)), sfcc_cimdata_to_value(&data, rso->client, false));
      }
      else {
        sfcc_rb_raise_if_error(status, "Can't retrieve key #%d", k);
      }
      if (key_name) CIMCRelease(key_name);
    }
  }
  else {
